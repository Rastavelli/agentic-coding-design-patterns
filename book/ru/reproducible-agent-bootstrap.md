...
Новая сессия входит с неизвестным локальным состоянием и вызывает единственную команду.
...
flowchart TB
  ...
  note["сначала фиксируем исходный результат;<br/>новые сбои сравниваем с ним"]:::accent
  smoke -.- note
```

Команда валидирует инструменты, создаёт безопасное состояние и запускает smoke-check. Только зелёный результат открывает работу над задачей; красный останавливает её и отделяет проблему среды от будущего диффа.
...
3. Создавайте конфигурацию из безопасного примера. Не копируйте реальные токены и не перезаписывайте существующий _.env_ без явного решения.
...
Для реального проекта каркас нужно уточнить. Условие сохраняет существующий файл конфигурации, а ошибка копирования останавливает `make`. Порт и Compose project должны различаться между worktree, а smoke-тест должен ждать готовности базы с ограниченным тайм-аутом.

После добавления проверки версии среды и формирования отчёта bootstrap может выдавать результат в следующем формате. Это образец желаемого отчёта, а показанный выше каркас его ещё не печатает.
...
- **Харнес Anthropic для долгоживущих агентов** использует initializer agent, который создаёт `init.sh`, а каждая следующая сессия запускает сервер и базовый end-to-end тест до новой работы.
