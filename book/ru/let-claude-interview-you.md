...
```mermaid
---
title: время на точность спеки окупается лучше наблюдения за реализацией
---
flowchart TB
...
В `SPEC.md` агент записывает это правило вместе с форматом событий, подписью и политикой повторов. Сквозная проверка должна создать заказ, дождаться события в тестовом приёмнике, затем воспроизвести сбой и проверить отключение. Свежая сессия получает требование, по которому можно написать код и тесты.
