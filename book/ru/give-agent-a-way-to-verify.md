...
Доказательство должно прийти из окружения, а не из рассказа агента. Вывод, который агент перепечатал в итоговое сообщение, он мог и сочинить: написать `4 passed` без запуска или по запуску до последней правки. Проверяйте по записи, которую сохранил харнес: вызову инструмента в журнале сессии, логу CI, коду возврата в хуке, скриншоту от браузерного инструмента. Эти данные показывают, что именно агент проверил, и помогают принять работу без восстановления всей сессии.

## Структура

На схеме разработчик передаёт задачу и критерии проверки.
...
flowchart TB
  ...
  evidence["Доказательства из окружения<br/>журнал запусков, лог CI,<br/>код возврата, скриншот"]:::accent
...
Агент повторяет цикл правок и проверок до успеха, затем возвращает доказательства. Хук может закрепить условие выхода, а независимое ревью дополнить автоматические проверки.
...
- **Доказательства** сохраняют команду, вывод или изображение проверенного состояния. Их записывает окружение, а не агент.
...
6. Принимайте работу по записям окружения: журналу вызовов инструментов, логу CI или результату хука. Пересказ результата в итоговом сообщении агента доказательством не считается.
7. Закрепите команды проверки в [памяти проекта](claude-md-memory.md), чтобы агент знал их в каждой сессии.

В [OpenSpec](openspec.md), [Superpowers](superpowers.md) и [скиллах Мэтта Покока](matt-pocock-skills.md) проверки входят в рабочий процесс реализации. Конкретный механизм зависит от набора скиллов и критериев задачи.
...
Агент пишет тесты и реализацию. Проверка просроченного кода падает, потому что сравнение дат не учитывает часовой пояс. После исправления агент повторяет запуск. В журнале сессии виден последний прогон тестов после финальной правки с результатом `4 passed`.
...
- **Пересказанный вывод.** Строка `4 passed` в итоговом сообщении — тоже слова агента. Сверяйте её с запуском в журнале сессии или в CI.
...
- **Юнит-тесты как финал.** Для пользовательской фичи проверьте также сквозной сценарий, иначе возможен [преждевременный успех](premature-success.md).
...
- **Харнес Anthropic для долгоживущих агентов** связывает статусы фич с проверками и запускает smoke-тест в начале сессии.
...
- [Преждевременный успех](premature-success.md) возникает, когда работу объявляют готовой без сквозной проверки пользовательского сценария.
- [Уан-шот](one-shotting.md) описывает ожидание готового результата от одного прохода без цикла проверки.
