...
Исполняемая политика задаёт конкретные границы этих действий.
...
При вызове `git push` система запрашивает подтверждение. Запись в `~/.ssh/config` она блокирует, а `make test` выполняет без вопроса. Разработчик принимает решение только там, где политика требует его участия.
