...
- Для **модели состояний** создайте небольшой инструмент с действиями и видимым состоянием. Терминал подходит разработчику; для обсуждения с экспертом удобно сделать отдельный HTML-файл с кнопками и сценариями.
...
Вопрос определяет форму эксперимента. После прогона вывод и экспериментальный код сохраняются раздельно.

```mermaid
---
title: наблюдение из прототипа становится основанием для решения
config:
  flowchart:
    rankSpacing: 30
---
flowchart TB
  question{"Что проверить?"}:::accent
  logic["Проверить логику"]
  ui["Сравнить UI-варианты"]
  run["Прогнать трудные сценарии"]
  decision@{ shape: doc, label: "Вывод в тикете или ADR" }
  branch["Прототип в отдельной ветке"]:::muted
  code["Рабочая реализация"]:::accent
  question -- "поведение модели" --> logic
  question -- "взаимодействие" --> ui
  logic --> run
  ui --> run
  run -- "наблюдения" --> decision
  run -- "сохранить эксперимент" --> branch
  decision -- "проверенные требования" --> code
```

Разработчик выполняет сценарии и записывает вывод со ссылкой на ветку прототипа. Рабочую реализацию создают по проверенным требованиям. Экспериментальный код остаётся свидетельством проверки в отдельной ветке.
...
- **Скиллы Мэтта Покока** реализуют эксперимент через [/prototype](https://github.com/mattpocock/skills/blob/main/skills/engineering/prototype/SKILL.md). Для логики скилл создаёт отдельный HTML-файл со свободными действиями и пошаговыми сценариями; для UI переключаемые варианты. Проверенный прототип сохраняется в отдельной ветке со ссылкой из задачи.
- **Spike solutions в экстремальном программировании** снимают технический риск коротким экспериментом.
- **[«Спроектируй дважды»](design-it-twice.md)** развивает принцип Джона Аустерхаута: сравнивать существенно разные варианты дизайна до реализации.
...
- [Спроектируй дважды](design-it-twice.md) сравнивает альтернативные конструкции; прототип проверяет вопросы, требующие наблюдений.
