...
На схеме спецификация разделяется на сквозные срезы со связями.
...
Исполнитель выбирает доступный тикет и проверяет его целиком. Нижняя часть показывает разбиение по слоям, при котором проверка совместного поведения откладывается до интеграции.
