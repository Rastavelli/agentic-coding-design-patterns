...
Например, запрос «добавить вебхуки заказов» не определяет поведение при медленном получателе. Во время реализации агенту придётся выбрать политику повторов самостоятельно. Длинное описание идеи может сохранить тот же пропуск. Нужен участник, который задаст вопрос о поведении при сбое.
...
Схема показывает переход от краткой идеи через вопросы к спецификации.
...
Граница перед реализацией означает передачу документа в новую сессию.
...
3. Попросите сохранить спецификацию в _SPEC.md_.
...
Когда разработчик не знает правила, выясните, кто может его объяснить и какое решение зависит от ответа. Попросите агента подготовить отдельный опросник с контекстом задачи, вопросами в порядке важности и местом для ответов. Такой документ можно передать эксперту, который не участвовал в беседе.

Например, перед миграцией биллинга финансовая команда должна уточнить правила возврата. В опроснике агент спрашивает, что происходит при частичном использовании периода и какие исключения предусмотрены договором. Ответ «не знаю» сохраняется как открытый вопрос. После получения ответов разработчик проверяет их и поручает агенту обновить требования.

Скилл [to-questionnaire](https://github.com/mattpocock/skills/blob/main/skills/productivity/to-questionnaire/SKILL.md) реализует подготовку такого опросника. Сначала он уточняет получателя и необходимые сведения. Отправка документа и включение ответов в спецификацию остаются отдельными шагами команды.
...
В _SPEC.md_ агент записывает выбранные условия вместе с форматом событий, подписью и политикой повторов. Например, команда согласовала такой фрагмент требования.
...
- **Скиллы Мэтта Покока** сохраняют результаты `/grill-with-docs` в CONTEXT.md и ADR.
