...
На схеме каждая задача проходит свой цикл в отдельном worktree.
...
Интегратор принимает коммиты по одному и проверяет собранное состояние. Пересечения задач обнаруживаются при обновлении и слиянии веток.
...
- Параллельные сессии запускаются локально или автоматическим харнесом.
...
### Разрешение конфликтов по намерениям

При конфликте попросите агента восстановить назначение обеих правок по коммитам, PR и исходным задачам. Он должен объяснить, какие требования сохраняет объединённый вариант. Если требования несовместимы, согласуйте нужное поведение до продолжения интеграции.

Например, одна ветка добавляет тайм-аут запроса, другая ограничивает число повторов. Выбор только одной стороны может потерять второе ограничение. После объединения проверьте оба поведения и их совместную работу. Даже автоматическое слияние Git требует этой проверки. Скилл [resolving-merge-conflicts](https://github.com/mattpocock/skills/blob/main/skills/engineering/resolving-merge-conflicts/SKILL.md) использует поиск исходных намерений; в индекс включайте только файлы текущей интеграции, сохраняя посторонние изменения.
