...
Без общего реестра новая сессия также тратит время на восстановление очереди задач. [Журнал прогресса](progress-file.md) объясняет ход работы и причины решений. Для статусов нужен отдельный структурированный файл, в котором агент меняет определённые поля.
...
На схеме требования превращаются в реестр до реализации.

```mermaid
---
title: проверка подтверждает статус фичи
---
flowchart TB
...
Агент выбирает из него пункт и проходит цикл разработки с проверкой. Пунктирная стрелка возвращает фичу в очередь, если позднее обнаружена регрессия.
...
Для небольшой задачи обычно хватает плана или _tasks.md_.
...
Агент строит сервис заметок. Начальная сессия разворачивает спецификацию в реестр с непройденными пунктами. Ниже показан фрагмент после проверки создания заметки. Поиск по тегам ещё не проверен.
...
- **Харнес Anthropic для долгоживущих агентов** использует реестр фич и проверку через браузер перед изменением статуса.
- **Эвал-харнесы** используют фиксированный набор сценариев, который защищают от подгонки под результат.
- **SDD-тулкиты** хранят задачи в _tasks.md_, как в [OpenSpec](openspec.md). Реестр дополнительно связывает отметку с проверкой поведения.
