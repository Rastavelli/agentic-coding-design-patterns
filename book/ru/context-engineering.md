...
Контекстное окно получает сведения из нескольких источников. Для продолжения работы решения сохраняются отдельно от истории диалога.

```mermaid
---
title: файлы сохраняют состояние между контекстными окнами
config:
  flowchart:
    rankSpacing: 30
---
flowchart TB
  rules@{ shape: doc, label: "Правила и словарь" }
  files@{ shape: docs, label: "Код и данные задачи" }
  current["Текущее окно<br/>инструкции · диалог · результаты"]:::accent
  saved@{ shape: doc, label: "Прогресс и решения" }
  next["Окно следующей сессии"]:::accent
  rules -- "при старте" --> current
  files -- "по мере необходимости" --> current
  current -- "записать" --> saved
  saved -- "прочитать при старте" --> next
  current -. "сжать историю в сводку" .-> current
```

Постоянные инструкции подключаются и к следующей сессии; код задачи она читает по необходимости. Журнал прогресса и handoff сохраняют решения вне окна. Пунктирная петля показывает сжатие истории внутри текущей сессии: отработанные результаты инструментов уступают место краткой сводке.
...
- **Раздутый файл памяти.** Среди сотен правил агенту труднее выделить применимые инструкции. Эта ошибка разобрана в главе [«Раздутая память»](bloated-claude-md.md).
...
- [Раздутая память](bloated-claude-md.md) описывает постоянный слой контекста, перегруженный дублями и устаревшими правилами.
