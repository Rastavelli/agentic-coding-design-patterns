...
На схеме текстовая инструкция направляет выбор агента, а механизм проверяет каждое действие.
...
Он разрешает допустимое, блокирует запрещённое и передаёт человеку случаи, требующие решения. После выполнения отдельная проверка подтверждает требования к результату.
