...
2. **План.** Агент описывает подход, порядок изменений и риски. Вы проверяете план и уточняете ограничения, прежде чем агент возьмётся за код.
...
Если во время работы над кодом выясняется, что в плане ошибка, агент возвращается к планированию и согласует с вами изменение. В этом варианте процесса вы явно утверждаете план, и только потом агент пишет код.
...
3. Прочитайте план. Уточните скрытые ограничения, обсудите альтернативы и вычеркните лишнюю работу.
4. Утвердите план и назовите команды, которыми агент проверит результат.
5. Попросите агента закоммитить результат, подготовить пулл-реквест и обновить документацию, которую затронули изменения.
...
- **Исследование и план.** Команды `/speckit.specify`, `/speckit.clarify`, `/speckit.plan` и `/speckit.tasks` по очереди записывают требования, уточнения, технический подход и задачи. Дополнительно `/speckit.analyze` проверяет, что документы не противоречат друг другу, а вы читаете их до начала работы над кодом.
...
В **плане** агент предлагает два варианта: преобразовывать время при записи или при чтении. Вы уточняете ограничение.
...
Вы утверждаете исправленный план. На фазе **кода** агент вносит изменение и запускает тесты экспортёра.
...
- **Утверждение плана без чтения.** Если вы одобряете план, не читая его, контрольная точка становится формальностью. Тогда паттерн только добавляет лишней работы к обычному «сделай».
