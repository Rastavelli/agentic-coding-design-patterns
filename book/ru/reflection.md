...
flowchart TB
  subgraph session["одна сессия — одно окно"]
    ...
    critique["Критика<br/>по заданным осям<br/>список слабых мест"]:::warn
    revise["Правка<br/>по списку"]
    draft --> critique --> revise
    revise -. "ещё раунд — не больше одного-двух" .-> critique
  end
  dev["Разработчик<br/>задаёт оси · читает список<br/>решает, что чинить"]:::accent
  result["Результат<br/>после фильтра"]:::accent
  caveat["предвзятость автора не устранена —<br/>серьёзный дифф проверяет свежий контекст"]:::warn
  dev --> critique
  revise --> result
  result -.- caveat
```
