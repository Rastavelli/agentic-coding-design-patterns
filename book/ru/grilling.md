...
Например, план миграции описывает обычную подписку, но пропускает корпоративные зачисления. Если не задать вопрос об этом до реализации, агент применит одно правило ко всем пользователям.
Обсуждение каждой ветки помогает обнаружить такой пропуск до изменения кода.
...
```mermaid
---
title: дыры всплывают за столом — по цене реплики, а не переделки
---
flowchart LR
...
На схеме агент получает готовый план и проходит по веткам решений. Найденный пропуск возвращает его к документу, а вопрос, требующий эксперимента, уходит в прототип. Завершение обсуждения требует подтверждения разработчика.
...
В плане этот сценарий отсутствует. Разбор выявляет спорный вопрос о модели отмен, для которого нужен [прототип](prototype-to-answer.md). Агент готовит [передачу сессии](handoff.md) с вопросом и вариантами эксперимента. К реализации миграции команда возвращается после проверки этого решения.
...
- **Скилы Мэтта Покока** реализуют этот процесс через `/grilling` и `/grill-with-docs`. [Карта исследования](wayfinder.md) использует гриллинг как один из типов работы.
