...
Без записи требования приходится восстанавливать по памяти автора.
Последующие правки могут ещё дальше увести поведение от цели, поскольку сравнивать его не с чем.

Принятие сгенерированного кода без такого сравнения описано в анти-паттерне [вайб-кодинга](vibe-coding.md).
...
```mermaid
---
title: "контрольные точки: разработчик ревьюит артефакт, а не дифф в конце"
---
flowchart TB
  ...
  impl -. "реальность разошлась со спецификацией —<br/>правим спецификацию, не только код" .-> spec
  rules -.- spec
  rules -.- impl
```

Схема связывает спецификацию, план, задачи и код. Постоянные соглашения проекта ограничивают выбор на каждой фазе. Обратная стрелка показывает пересмотр требований по новым сведениям.
...
- [Superpowers](superpowers.md) связывает фазы через скилы и обязательные контрольные точки.
- [Скилы Мэтта Покока](matt-pocock-skills.md) сохраняют спецификации и трассирующие тикеты в трекере.
...
- [OpenSpec](openspec.md), [Superpowers](superpowers.md) и [скилы Мэтта Покока](matt-pocock-skills.md) разобраны в этом разделе. Общий подход также описан в [анонсе Spec Kit](https://github.blog/ai-and-ml/generative-ai/spec-driven-development-with-ai-get-started-with-a-new-open-source-toolkit/).
