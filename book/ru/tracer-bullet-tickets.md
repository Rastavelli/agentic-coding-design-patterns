...
Tracer-bullet tickets, вертикальные срезы, трассеры; `/to-tickets` в скилах Мэтта Покока.
...
Например, после создания всей схемы и API пользователь ещё не может настроить экспорт. Ошибка согласования интерфейсов обнаружится только после готовности UI.
Узкий сценарий создания одного расписания позволяет проверить взаимодействие слоёв раньше.
Для следующего сценария нужно явно указать зависимость от уже работающего создания расписания.
...
```mermaid
---
title: узкий сквозной срез даёт проверяемое поведение
---
flowchart LR
  spec["Спецификация<br/>утверждённый эпик"]:::accent
  subgraph slices["каждый тикет — срез через схему, API, UI и тесты"]
    direction LR
    t1["тикет 1"]:::accent
    t2["тикет 2"]:::accent
    t3["тикет 3"]:::accent
  ...
  frontier["Фронтир<br/>тикеты без блокеров;<br/>по одному на сессию,<br/>каждый приземляется зелёным"]
  horizontal["для контраста: горизонтальная нарезка —<br/>широкий кусок одного слоя —<br/>не демонстрируема до интеграции в самом конце"]:::warn
  spec --> slices --> frontier
  t2 -.- horizontal
```

На схеме спецификация разделяется на сквозные срезы со связями. Исполнитель выбирает доступный тикет и проверяет его целиком. Нижняя часть показывает разбиение по слоям, при котором проверка совместного поведения откладывается до интеграции.
...
- **Скилы Мэтта Покока** используют `/to-tickets` для сквозного разбиения и `/implement` для выполнения тикетов.
