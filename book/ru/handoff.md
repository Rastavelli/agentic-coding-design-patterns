...
Handoff, хендофф; `/handoff` в скиллах Мэтта Покока; передаточный документ.
...
Автоматическая сводка может сохранить ход обсуждения, но пропустить причину отказа от одного варианта. Новый агент тогда повторит уже проделанное исследование. Ручной пересказ тоже требует времени и зависит от памяти разработчика.
...
- рекомендации по скиллам и инструментам следующей сессии.
...
Верхний путь на схеме показывает передачу под заданную цель.
...
flowchart LR
  ...
  a --> doc --> b
  doc -.- artifacts
  compact["Сессия A продолжается<br/>история заменена сводкой"]:::muted
  a -. "автоматическое сжатие" .-> compact
  note["что переживёт границу сессии,<br/>решает разработчик"]:::muted
  b -.- note
```

Текущий агент составляет документ, а следующий читает его и обращается к постоянным артефактам по ссылкам. Нижний путь показывает автоматическое сжатие внутри текущего разговора, при котором разработчик меньше контролирует состав сводки.
...
Агент сохраняет _handoff-cancellation-prototype.md_ во временном каталоге.
...
- **Скиллы Мэтта Покока** используют `/handoff` для передачи между этапами, включая переход от интервью к прототипу.
