...
**Глоссарий** в _CONTEXT.md_ задаёт термины домена. Для него достаточно короткого формата.
...
**Журнал решений** в _docs/adr/_ сохраняет принятое решение и его причину в отдельном файле. В этом варианте паттерна ADR нужен для выбора, который трудно обратить и трудно понять без контекста. Запись должна объяснять, какие альтернативы команда рассматривала и почему выбрала одну из них. Для простого случая достаточно абзаца.
...
На схеме глоссарий и журнал решений поступают в контекст агента.
...
Если термин задачи расходится со словарём, агент запрашивает уточнение у разработчика и сохраняет принятое определение. Пунктирная стрелка показывает это обновление. Для проекта с несколькими доменами _CONTEXT-MAP.md_ указывает, где находятся словари и как связаны их контексты.

## Участники / Компоненты

- **Глоссарий** (_CONTEXT.md_) хранит определения и нежелательные синонимы.
- **Журнал решений** (_docs/adr/_) объясняет неочевидные архитектурные выборы.
- **Карта контекстов** (_CONTEXT-MAP.md_) связывает словари нескольких доменов.
...
1. Создайте _CONTEXT.md_, когда согласован первый термин, и _docs/adr/_, когда появилось решение, которое нужно объяснить будущему читателю.
...
7. В проекте с несколькими доменами разместите словари рядом с модулями и свяжите их через _CONTEXT-MAP.md_.

В [скиллах Мэтта Покока](matt-pocock-skills.md) скилл `domain-modeling` ведёт CONTEXT.md и ADR. Остальные скиллы используют эти документы при интервью, подготовке спецификации и реализации.
...
Причину выбора имени сохраняет ADR _docs/adr/0003-enrollment-not-subscription.md_.
...
- **Скиллы Мэтта Покока** реализуют паттерн через `domain-modeling`, который ведёт словарь, ADR и карту контекстов.
