...
Автор и рецензент работают в разных контекстах. Между ними передаются артефакты проверки и находки.

```mermaid
---
title: независимое ревью требует свежего контекста
config:
  sequence:
    mirrorActors: false
    width: 130
...
---
sequenceDiagram
  participant W as Автор (A)
  participant R as Рецензент (B)
  Note over W: История автора<br/>остаётся здесь
  W->>R: Дифф + требования + ссылки на код
  R->>R: Проверить требования<br/>и контрпримеры
  R-->>W: Находки с доказательствами<br/>или отсутствие находок
  opt Дефекты подтверждены
    W->>W: Исправить и проверить
    W->>R: Обновлённый дифф
    R-->>W: Результат повторной проверки
  end
```

Сессия B начинается со свежего контекста: рассуждения автора в неё не переносят. Рецензент читает нужный код и сообщает находки. Исправления остаются у автора и проходят повторную оценку.
